...
### RLS is enabled on every table, with zero policies — and that's deliberate, not a gap to fill

Confirmed directly against the real Supabase project (August 2026, via `get_advisors`): every `public` schema table has row-level security **enabled**, and **not one policy exists anywhere.** Nothing in this codebase turned that on — `grep` across `packages/db/migrations/*.sql` for `ROW LEVEL SECURITY` returns nothing. It's a Supabase platform default, applied automatically to tables in the exposed API schema.

**This is currently harmless, not currently correct-by-accident.** `dbDirect()` and `dbPooled()` (below) both connect as the `postgres` role, which has `BYPASSRLS` — Postgres's RLS mechanism does not apply to a table's owner or a superuser regardless of policy count, so "enabled, no policies" denies nothing to the only thing querying these tables today. The advisor still reports it (`rls_enabled_no_policy`, INFO-level, one finding per table) because the check has no way to know that; it isn't wrong, it's just describing PostgREST's-eye view of the table, and PostgREST is a path this application never takes (§5, above).

**State this plainly so a future session doesn't "fix" it by writing policies out of habit: RLS is deliberately not the authorization model here.** Every authorization decision is a code-level check in a Server Component, Server Action, or route handler, per §4.1 and the paragraph above. Writing RLS policies wouldn't add safety to anything Drizzle touches — it would just be a second, unused enforcement layer sitting behind the one that actually runs.

**The warning worth carrying forward, from §7 (Auth):** the moment Supabase Auth ships and any browser client queries a table directly with a user's JWT — not through Drizzle, not through `postgres`'s `BYPASSRLS` — RLS stops being inert. A table with RLS enabled and no policies denies that request **everything**, silently and by design (Postgres's own default posture: enabled + no matching policy = no rows, no error). That will present as a confusing application bug — "the query returns nothing" or "insert is refused" with no exception explaining why — not as an RLS error message pointing at itself. When that day comes, exactly one of two things must happen, decided on purpose rather than discovered via a support ticket: either real policies get written for whatever a browser client is now allowed to touch directly, or the rule holds firm and **browser clients never query Postgres tables directly at all** — every such need goes through a Server Action or route handler instead, and the browser's only direct Postgres-adjacent path stays the one that already exists on purpose: the public, unauthenticated Realtime broadcast channel (§8), which was never gated by RLS in the first place because it was never meant to be.
...
Supabase Auth, not hand-rolled. **Partially built, 2026-08-10 — password sign-up/sign-in/sign-out, session handling, and role-gating plumbing exist; Google/Apple, dependent promotion, and email one-tap actions don't yet.** What follows describes the current shape, corrected from the original design-only draft where implementation surfaced a difference.
...
**The `credentials` table design already anticipated this.** Per `DOMAIN-MODEL.md` §2, credentials are deliberately a separate table so "any dependent may eventually want their own login... promotion is an insert, never a data migration." That's exactly the shape Supabase Auth needs: a `credentials` row with `provider_subject = auth.users.id`, inserted on first sign-in or on dependent promotion. **Correction from the original draft of this section:** `provider` is not literally `'supabase'` — the schema's four-value enum (`password` / `google` / `apple` / `email_link`) is more specific than that, and more correct: it's *which method* Supabase Auth used, matching what `credentials` needs to mean. `'supabase'` would have been a fifth, redundant value the check constraint doesn't even allow. A dependent with no `credentials` row has no login — true whether or not Supabase Auth exists behind it.

**Built:**

- `apps/web/app/lib/supabase/server.ts` / `browser.ts` — the two `@supabase/ssr` client factories, `createServerClient`/`createBrowserClient`, current `getAll`/`setAll` cookie pattern (verified against Supabase's own docs and this app's actual installed `@supabase/ssr` version at implementation time, not from memory — the older three-method `get`/`set`/`remove` pattern shown in older material is superseded).
- `apps/web/proxy.ts` — refreshes the session cookie on every request via `getClaims()` (never `getSession()`, which Supabase's docs are explicit doesn't re-validate). This is Next 16's renamed `middleware.ts` convention — see §2's build-step note and `apps/web/AGENTS.md`. Deliberately does *not* redirect unauthenticated requests anywhere; every route/action re-checks itself instead (below), matching Next's own data-security guidance that a proxy-level or page-level check never extends to a nested Server Action or route handler.
- `packages/core/identity.ts` — `ensurePersonForAuthUser` (the `credentials` insert on first sign-in, idempotent) and `getPersonForAuthSubject`/`getPersonRoles`/`personHasRole` (the read side). Domain-layer, takes a `tx`, like everything else in this package — see §4.1.
- `apps/web/app/lib/auth.ts` — composes a validated session (`getClaims()`) with the domain lookup above into `getCurrentPerson()`/`requireCurrentPerson()`/`requireOwnerOrRole()`, the three things a route handler or Server Action actually calls. Every one of the three season-registration routes (§ this doc doesn't number routes, see `apps/web/app/api/registrations/**`) now calls one of these itself, closing the gap those routes shipped with when they were built ahead of auth on direct instruction (`docs/DECISIONS.md`, 2026-08-10).
- Password sign-up/sign-in/sign-out: `apps/web/app/signup/`, `apps/web/app/login/`, `apps/web/app/lib/auth-actions.ts`. The "I am 16 or over" attestation (`DOMAIN-MODEL.md` §2) is a required checkbox at signup, timestamped into `is_adult_attested_at` by `ensurePersonForAuthUser`.

**Not built yet:**

- Google, Apple providers — password only so far. Apple is still required on iOS once any other third-party sign-in ships (unchanged from the original draft).
- Dependent promotion (`DOMAIN-MODEL.md` §2) and any guardian-acts-for-dependent authorization. `requireOwnerOrRole` only knows "is this the resource's own person, or an admin" — a guardian acting for a dependent isn't either of those yet.
- Email one-tap actions (confirm/decline via a link, no login) — still meant to stay a **separate** mechanism from Supabase Auth sessions (signed, single-purpose, expiring tokens), not built.
- Multi-provider identity merging — the same human signing in with both password and a future OAuth provider. `ensurePersonForAuthUser` looks up by `(provider, provider_subject)`, so two different providers for one person currently provision two different `people` rows rather than linking. A real gap once a second provider ships, not handled by assumption.
- Native: nothing to build yet — `apps/mobile` doesn't exist (phase 12).

**A stale part of the schema, found while building this:** `DOMAIN-MODEL.md` §2's `sessions` table (`token_hash`, `expires_at`, `revoked_at`) is the self-hosted plan's hand-rolled session design, never updated when auth pivoted to Supabase — this section's own "use Supabase Auth's own refresh-token flow... rather than reimplementing it" already said not to build that, it just didn't say what to do with the table that already existed for it. **`sessions` is not used by anything built this session and shouldn't be** — Supabase Auth's own JWT/cookie session is authoritative. Left in the schema rather than dropped, since dropping a table is more disruptive than leaving one unused, but flagged here so a future session doesn't wire it in out of habit or assume it's load-bearing because it's migrated.
