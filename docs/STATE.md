...
`declineOffer` for season registration in `packages/core`, a new
`seasons.offer_window_minutes` migration, and 14 passing integration tests.
...
- **Concurrency core — `hold`/`confirm`/`release`/`promote`/`declineOffer` for season registration, against real local Postgres.** 14 `node:test` integration tests, no mocks: mixed carts (one line held, one waitlisted, correct total), a full slot waitlisting instead of failing, waitlist queue position ordering, re-registering an already-held slot coming back as a clean outcome rather than a duplicate row, idempotent webhook confirmation, a release freeing its spot for the next holder, `promoteWaitlist` picking the earliest queued registration with the season's configured offer window, `declineOffer` re-queuing the decliner at the **back** of the queue and promoting the next person in the same call, and `promoteWaitlist` sweeping a lapsed (never-responded) offer back into the queue before promoting whoever's next. That last pair of tests caught a real bug before it shipped: the sweep originally stamped the re-queued row's timestamp with SQL `now()` (a transaction's start time) while every other timestamp in the module comes from JS `Date.now()`, so a long-running transaction's sweep could out-rank someone who'd genuinely been waiting longer — fixed by sourcing the sweep's timestamp from JS like everywhere else. Plus one **real independent-connection concurrency test** — 8 separate transactions on separate connections racing `Promise.all` against a 1-capacity slot — proving the row lock serializes actual concurrent connections, not just sequential calls sharing one transaction. This is **not** the load-test gate itself (`ARCHITECTURE.md` §12 wants several hundred concurrent multi-line carts against a 20-capacity slot) — it's a fast sanity check that the mechanism the load test will exercise actually holds.
...
- Accepting a waitlist offer — creating the one-line `registration_carts` row on acceptance is a payment-flow function belonging with `holdCart`/`confirmCart`, not built this session; needs a route layer that doesn't exist yet.
