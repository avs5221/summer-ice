...
(the next day, not part of the design-import session below) unifies the
theme toggle to one placement across all six restyled pages —
`position: absolute` against `.page`, not `fixed` and not embedded in
`SiteFooter` — per direct product feedback, and fixes a real
click-blocking bug on `/register` found by testing rather than
inspection (its fixed checkout bar was intercepting the toggle's
clicks). The design-import session before it, each on its own commit:
